document.addEventListener('DOMContentLoaded', function () {
  ...
  function createNewTaskElement(element) {
    ...
    tdCompanyName.className = 'company';
    tdContactName.className = 'contact';
    ...
    editButton.innerHTML = '<i class="fa fa-pen icon-pen">';
    ...
    // filter
    function filterFunction(filterName, searchText) {
      let tdName = tr.querySelectorAll('.' + filterName); 
      for (i = 0; i < tdName.length; i++) {                
        if (tdName[i].innerHTML.toUpperCase().indexOf(searchText.toUpperCase()) > -1) {
          console.log(tdName);
          tdName[i].style.display = '';
        } else {
          tdName[i].closest('tr').style.display = 'none';
        }
      }
    }

    const input = document.querySelector('.filter-button');
    const inputFilters = document.querySelectorAll('.input');
    input.addEventListener('click', function () {
      let isFiltersFillded = false;
      for (i = 0; i < inputFilters.length; i++) {
        let val = inputFilters[i].value;
        if (val.trim() != '') {
          isFiltersFillded = true;  
          let filterName = inputFilters[i].getAttribute('data-filter');
          filterFunction(filterName, val.trim());
        }
      }
    ...
    // Show all rows.
    function showAll() {
      let trs = document.querySelectorAll('tr.add-tr');
